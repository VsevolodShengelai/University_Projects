...
area = Label(canvas, text="Площадь, м²", width=14)
...
coeff.place(x=10, y=310)
area.place(x=10, y=370)
...
Coeff.place(x=10, y=334)

Area = Spinbox(canvas, from_=0, to=100, width=15)
Area.place(x=10, y=394)
...
def InButton6_pressed():
    ...
    if k >= 0:
        ...
        InfoPanel.insert(END, ">Ошибка:Указано недопустимое значение коэф-та сопротиаления k\n")
        InfoPanel.see("end")


def InButton7_pressed():
    global S
    S = float(Area.get())
    if S >= 0:
        InfoPanel.insert(END, ">Площадь = " + str(S) + " \n")
        InfoPanel.see("end")
    else:
        InfoPanel.insert(END, ">Ошибка:Указано недопустимое значение площади S\n")
        InfoPanel.see("end")
...
def ResetButton7_pressed():
    global S
    S = None
    InfoPanel.insert(END, ">Сброс площади" + "\n")
    InfoPanel.see("end")
...
InButton7 = Return_InButton_standart(InButton7_pressed)
ResetButton7 = Return_ResetButton_standart(ResetButton7_pressed)

InButton_vel_window = canvas.create_window(120, 12, anchor=NW, window=InButton1)
InButton_ang_window = canvas.create_window(120, 72, anchor=NW, window=InButton2)
InButton_mas_window = canvas.create_window(120, 132, anchor=NW, window=InButton3)
InButton_itr_window = canvas.create_window(120, 192, anchor=NW, window=InButton4)
InButton_grv_window = canvas.create_window(120, 252, anchor=NW, window=InButton5)
ResetButton_vel_window = canvas.create_window(160, 12, anchor=NW, window=ResetButton1)
ResetButton_ang_window = canvas.create_window(160, 72, anchor=NW, window=ResetButton2)
ResetButton_mas_window = canvas.create_window(160, 132, anchor=NW, window=ResetButton3)
ResetButton_itr_window = canvas.create_window(160, 192, anchor=NW, window=ResetButton4)
ResetButton_grv_window = canvas.create_window(160, 252, anchor=NW, window=ResetButton5)
AutoButton_coeff_window = canvas.create_window(120, 312, anchor=NW, window=AutoButton6)
InButton_coeff_window = canvas.create_window(153, 312, anchor=NW, window=InButton6)
ResetButton_coeff_window = canvas.create_window(186, 312, anchor=NW, window=ResetButton6)

InButton_area_window = canvas.create_window(120, 372, anchor=NW, window=InButton7)
ResetButton_area_window = canvas.create_window(160, 372, anchor=NW, window=ResetButton7)

# Информационная панель

InfoPanel = Text(canvas, bg="gray25", bd=2,
                 width=26, height=5, fg="green2")
InfoPanel_window = canvas.create_window(9, 432, anchor=NW, window=InfoPanel)
...
def Pusk_with_coeff():
    ...
    selfk = k
    selfS = S
    if U == None or a == None or m == None or t == None or g == None or k == None or S == None:
        InfoPanel.insert(END, ">Пуск невозможен" + "\n")
        InfoPanel.see("end")
    else:
        k = k * S * 1.2  # коэффициент обтекания, плотность воздуха и площадь поверхности
...
PuskButton = Button(canvas, text="Пуск с \n учётом а.с.", bg="lime green",
                    activebackground="Light Yellow2",
                    width=10, height=2, command=Pusk_with_coeff
                    )
...
PuskButton2 = Button(canvas, text="Пуск \n в вакууме", bg="lime green",
                     activebackground="Light Yellow2",
                     width=10, height=2, command=Pusk_in_vacuum
                     )
PuskButton_window = canvas.create_window(126, 540, anchor=NW, window=PuskButton2)
